...
type Message = {
  role: "user" | "assistant";
  content: string;
  _id?: Id<"messages">; // Convexのメッセージ ID
  chatId?: Id<"chats">; // チャットID
  createdAt?: number; // 作成日時
  sceneData?: string; // シーンデータ（JSON文字列）
};

interface ChatPageClientProps {
  chatId: string;
  initialChat: any; // チャットの型は実際のConvexスキーマに合わせて調整してください
  initialMessages: any[]; // メッセージの型も同様
}
...
export default function ChatPageClient({
  ...
  const chat = useQuery(api.chats.getChat, { id: convexChatId }) || initialChat;
  const rawMessages =
    useQuery(api.messages.listMessages, { chatId: convexChatId }) ||
    initialMessages;
  ...
  const [, setDebugText] = useState<string>(""); // デバッグ情報表示用
  ...
  const handleSendMessage = async () => {
    ...
    try {
      // 現在のメッセージ履歴を作成
      const messageHistory = [...formattedMessages, userMessage];
